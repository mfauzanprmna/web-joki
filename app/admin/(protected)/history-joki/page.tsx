...
export default async function AdminJokiHistoryPage() {
  const [games, entries] = await Promise.all([
    ...
    prisma.jokiHistoryEntry.findMany({
      include: { game: true, testimonial: { select: { id: true } } },
  ...
  return (
    <div>
      ...
      <p className="text-shihu-muted text-sm mb-6">
        Catat pengerjaan joki yang tidak lewat alur Pesanan biasa (mis.
        transaksi lama/di luar sistem) supaya tetap tampil di halaman History
        publik, lengkap dengan screenshot bukti pengerjaan. Setiap entri dapat
        link testimoni sendiri yang bisa disalin & dikirim ke customer-nya.
      </p>

      <details
        className="bg-shihu-card border border-shihu-border rounded-2xl p-5 mb-7 group"
      ...
      >
        ...
        <CreateJokiHistoryForm games={games} />
      </details>
      ...
      {entries.length === 0 ? (
        ...
        <div className="flex flex-col gap-2.5">
          {entries.map((e) => (
            <JokiHistoryRowItem
              key={e.id}
              item={{ ...e, hasTestimonial: Boolean(e.testimonial) }}
            />
          ))}
        </div>
      )}
